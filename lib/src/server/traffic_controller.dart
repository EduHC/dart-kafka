...
import '../common/message_header.dart';
...
class TrafficControler {
  factory TrafficControler({
    required Cluster cluster,
    required StreamController<dynamic> eventController,
    required KafkaAdmin admin,
    required KafkaClient kafka,
  }) {
    _instance ??= TrafficControler._(
      cluster: cluster,
  ...
  TrafficControler._({
    ...
    errorMiddleware = ErrorMiddleware(kafka: kafka, admin: admin);
  }
  ...
  late final ErrorMiddleware errorMiddleware;

  static TrafficControler? _instance;
  ...
  Future<({bool hasError, Map<String, dynamic>? errorInfo})>
      _messageInterceptor({required dynamic entity}) async =>
          errorMiddleware.hasError(entity: entity);
